...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    if (this._state && options.session) {
    ...
    this._oauth2.getOAuthAccessToken(code, params,
      function(err, accessToken, refreshToken, params) {
        if (err || params.errcode) {
          return self.error(self._createOAuthError('Failed to obtain access token', err || params));
        }
...
Strategy.prototype.tokenParams = function(options) {
  return { appid: this._appid, secret: this._secret };
};
...
Strategy.prototype.userProfile = function(accessToken, params, done) {
  ...
  this._oauth2.get(userProfileURL, accessToken, function(err, result) {
    if (err || result.errcode) { return done(new InternalOAuthError('Failed to fetch user profile', err || result)); }
